...
|---|---------|-------------|------------|-----------|---------|------------|
| 1 | `from` import with symbols | `from ./zig/core.zig import type` | `pub const type = @import("./zig/core.zig").type;` | `statement_import`: populate `StatementImport.symbols` with `Alias` chain for `nkFromStmt` children | `statement_import`: when `symbols` present, emit `pub const <sym> = @import("<path>").<sym>;` per symbol | medium |
| 2 | `from` import with `as` alias | `from ./zig/core.zig import type as Engine` | `pub const Engine = @import("./zig/core.zig").type;` | parse `as` in `nkFromStmt` → `Alias.name` = original, `Alias.target` = alias | codegen: use `Alias.target` as const name when present | medium (builds on #1) |
...
| 5 | Error union return type `!T` | `proc main *() :!void=` | `pub fn main() !void {` | needs preprocessing or special handling — `!void` is not valid Nim type syntax; could preprocess to a pragma or synthetic type annotation | codegen: emit `!` prefix before return type when marked as error union | medium |
