...
proc return_affix *(input_type :static string = "int") :TestData=
  ...
  let type_id   = result.ast.add_type(astTF.Type(kind: astTF.tPrimitive, primitive: astTF.TypePrimitive(name: astTF.Identifier(location: type_loc))))
  let type_expr = result.ast.add_expression_type(type_id)
  let arg2_id   = result.ast.add_binding(astTF.Binding(name: some(astTF.Identifier(location: arg2_loc)), dataType: some(type_expr), private: some(true), runtime: some(true)))
  let arg1_id   = result.ast.add_binding(astTF.Binding(name: some(astTF.Identifier(location: arg1_loc)), dataType: some(type_expr), private: some(true), runtime: some(true), next: some(arg2_id)))
