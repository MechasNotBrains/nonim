...
proc make_depth (state :var State; node :PNode) :astTF.Id=
  result = state.ast.add_depth(astTF.Depth(
...
#___________________
proc pragma_key (state :var State; pragma_node :PNode; caller :string= "") :astTF.Id=
  let key_node = case pragma_node.kind
    of nkExprColonExpr: pragma_node[0]
    else              : pragma_node
  result = state.expression(key_node)
#___________________
proc pragma_value (state :var State; pragma_node :PNode; caller :string= "") :Option[astTF.Id]=
  if pragma_node.kind == nkIdent: return none(astTF.Id)
  let value_node = case pragma_node.kind
    of nkExprColonExpr : pragma_node[1]
    else               : pragma_node
  result = some(state.expression(value_node))
#___________________
proc pragmas_binding (state :var State; name_node :PNode) :Option[astTF.Id]=
  result = none(astTF.Id)
  if name_node.kind != nkPragmaExpr: return
  var previous_pragma = none(astTF.Id)
  let pragma_node     = name_node[1]
  if pragma_node.kind == nkPragma:
    for pragma_child in pragma_node:
      let pragma_id = state.ast.add_pragma(astTF.Pragma(
        key         : state.pragma_key(pragma_child),
        value       : state.pragma_value(pragma_child),  ))
      if result.isNone:
        result = some(pragma_id)
...
proc type_node_to_type_id (state :var State; node :PNode; mutable = false) :astTF.Id=
  ...
  of nkPtrTy:
    ...
    return state.ast.add_type(astTF.Type(
      kind  : astTF.tPtr,
      `ptr` : astTF.TypePtr(target: target_id, mutable: some(mutable)),
    ))
  of nkProcTy:
...
proc expression_dot_leading (state :var State; node :PNode) :astTF.Id=
  ...
  result = state.expression_tuple(node, first=1)
...
proc expression_tuple (state :var State; node :PNode; first :int = 0) :astTF.Id=
  let has_names = node.safeLen > first and node[first].kind == nkExprColonExpr
  if has_names:
    var first_field = none(astTF.Id)
    var previous_field = none(astTF.Id)
    discard state.scope_push()
    for i in first ..< node.safeLen:
      let child = node[i]
...
proc expression_parenthesis (state :var State; node :PNode) :astTF.Id=
  ...
  result = state.expression_tuple(node)

proc expression_obj_constr (state :var State; node :PNode) :astTF.Id=
  var first_field    = none(astTF.Id)
  var previous_field = none(astTF.Id)
  let is_anonymous = node[0].kind == nkEmpty
  if is_anonymous: discard state.scope_push()
  for i in 1 ..< node.safeLen:
    let child = node[i]
    if child.kind != nkExprColonExpr or child.safeLen < 2: continue
    let name_loc = state.name_add(child[0].name())
    let value_id = state.expression(child[1])
    let depth_id = if is_anonymous: some(state.make_depth(child)) else: none(astTF.Id)
    let binding  = astTF.Binding(
      name       : some(astTF.Identifier(location: name_loc)),
      value      : some(value_id),
      runtime    : some(true),
    ...
    if previous_field.isSome:
      ...
      state.ast.data.bindings.get[previous_field.get] = prev
    previous_field = some(binding_id)
  if is_anonymous: state.scope_pop()
  ...
  for section in body_node:
      ...
      if section.kind == nkFromStmt:
        ...
        for symbol_index in 1 ..< section.safeLen:
          ...
          let last_dot     = symbol_name.rfind('.')
          let short_name   = if last_dot >= 0: symbol_name[last_dot + 1 .. ^1] else: symbol_name
          let binding_name = if alias_name.len > 0: alias_name else: short_name
...
proc procedure_build (state :var State; node :PNode) :astTF.Id=
  ...
  let name_str    = name_node.name()
  if name_str.len > 0: name_ident = some(astTF.Identifier(location: state.name_add(name_str)))
...
proc expression_case (state :var State; node :PNode) :astTF.Id=
  let subject_id = state.expression(node[0])
  var first_branch = none(astTF.Id)
  var previous_branch = none(astTF.Id)
  for branch_index in 1 ..< node.safeLen:
    let branch_node = node[branch_index]
...
proc variables_from_vartuple (state :var State; definition :PNode; is_mutable, is_runtime :bool; inside_body :bool = false) :seq[astTF.Id]=
  ...
  for name_index in 0 ..< name_count:
    ...
    let depth_id = case inside_body
      of true  : some(state.make_depth(name_node))
      of false : none(astTF.Id)
...
proc statement_keyword (state :var State; node :PNode) :astTF.Id=
  ...
  var value       = none(astTF.Id)
  var label       = none(astTF.Identifier)
  if node.kind in {nkBreakStmt, nkContinueStmt} and node.safeLen > 0:
    let label_node = node[0]
    if label_node.kind != nkEmpty:
      let label_loc = state.name_add(label_node.name())
...
proc statement_body (state :var State; node :PNode) :astTF.Id=
  ...
  proc body_case (state :var State; child :PNode) :astTF.Id=
    let cond_expr_id = state.expression_case(child)
    let depth_id = some(state.make_depth(child))
    result = state.ast.add_statement(astTF.Statement(
      kind       : astTF.sExpression,
      expression : astTF.StatementExpression(id: cond_expr_id, depth: depth_id),  ))
  ...
  proc body_it (state :var State; child :PNode) =
    ...
    let lambda_bind   = state.ast.add_binding(astTF.Binding(
      value           : some(lambda_id),
      runtime         : some(true),  ))
...
proc statement_type_object_fields (state :var State; body_node :PNode) :Option[astTF.Id]=
  result       = none(astTF.Id)
  let rec_list = body_node[2]
  var previous = none(astTF.Id)
  if rec_list.kind != nkRecList: return
  discard state.scope_push()
  for field_def in rec_list:
    ...
    for name_index in 0 ..< type_index:
      ...
      let is_last_in_group = name_index == type_index - 1
      let depth_id         = some(state.make_depth(field_name_node))
      ...
      if is_generic:
        ...
        let alias_key_id  = state.ast.add_expression(astTF.Expression(
          kind       : astTF.eIdentifier,
          identifier : astTF.ExpressionIdentifier(name: astTF.Identifier(location: alias_key_loc)),
        ))
        let alias_value_id  = state.expression(alias_value)
        let alias_pragma_id = state.ast.add_pragma(astTF.Pragma(key: alias_key_id, value: some(alias_value_id)))
        field_binding.pragmas = some(alias_pragma_id)
        field_binding.value   = some(alias_value_id)
      ...
      if result.isNone: result = some(field_id)
      if previous.isSome:
  ...
  if body_node.kind == nkObjectTy:
    ...
    for field_index in 0 ..< body_node.safeLen:
      ...
      if alias_value != nil:
        ...
        let alias_pragma_id = state.ast.add_pragma(astTF.Pragma(key: alias_key_id, value: some(alias_id)))
        let depth_id = some(state.make_depth(field_node))
        let binding_id = state.ast.add_binding(astTF.Binding(
    ...
    state.statement_chain(statement_id)
  elif body_node.kind == nkPtrTy and body_node[0].kind == nkProcTy:
    let proc_type_id = state.procedure_type(body_node[0])
    let ptr_type_id = state.ast.add_type(astTF.Type(
      kind  : astTF.tPtr,
      `ptr` : astTF.TypePtr(target: proc_type_id),
    ))
    let alias_type_id = state.ast.add_type(astTF.Type(
      kind  : astTF.tAlias,
      alias : astTF.TypeAlias(
        name    : some(astTF.Identifier(location: name_loc)),
        target  : state.ast.add_expression(astTF.Expression(
          kind   : astTF.eType,
          `type` : astTF.ExpressionType(id: ptr_type_id),
        )),
        private : some(is_private),
      ),
    ))
    let statement_id = state.ast.add_statement(astTF.Statement(
      kind   : astTF.sType,
      `type` : astTF.StatementType(id: alias_type_id),
    ))
    state.statement_chain(statement_id)
  else:
...
proc statement_describe (state :var State; node :PNode) :void=
  ...
  let desc_lit_id     = state.ast.add_expression(astTF.Expression(
    ...
    literal           : astTF.ExpressionLiteral(
      kind            : astTF.LiteralKind.string,
      value           : desc_loc,),))
  let desc_bind_id    = state.ast.add_binding(astTF.Binding(
    value             : some(desc_lit_id),
    runtime           : some(true),  ))
  let call_id         = state.ast.add_expression(astTF.Expression(
    kind              : astTF.eCall,
    call              : astTF.ExpressionCall(
...
proc statement_namespace (state :var State; node :PNode) =
  ...
  let body_node   = node[node.safeLen - 1]
  let name_str    = name_node.name()
  let is_private  = name_node.has_private_pragma()
  let keyword_loc = state.name_add("struct")
  discard state.scope_push()
  let saved_previous  = state.previous_stmt
  state.previous_stmt = none(astTF.Id)
  var first_stmt      = none(astTF.Id)
    ...
    for section in body_node:
      ...
      for definition in section:
        ...
        if value_node.kind != nkEmpty:
          value = some(state.expression(value_node))
        for name_index in 0 ..< name_count:
          let def_name_node = definition[name_index]
          let def_name_str  = def_name_node.name()
          if def_name_str.len == 0: continue
          let member_loc = state.name_add(def_name_str)
          let depth_id = some(state.make_depth(def_name_node))
          let binding_id = state.ast.add_binding(astTF.Binding(
            name     : some(astTF.Identifier(location: member_loc)),
            private  : some(state.declaration_private(def_name_node)),
            mutable  : some(is_mutable),
            runtime  : some(is_runtime),
            dataType : data_type,
            value    : value,
            pragmas  : state.pragmas_binding(def_name_node)
          ))
          let stmt_id = state.ast.add_statement(astTF.Statement(
            kind     : astTF.sVariable,
            variable : astTF.StatementVariable(id: binding_id, depth: depth_id),
...
proc statement_top_level (state :var State; node :PNode) =
  ...
  if state.target == Language.Zig and not state.typed:
    ...
    if node.is_at_prefix("describe") and node[node.safeLen - 1].kind == nkStmtList:
      state.statement_describe(node)
      return
    if node.is_at_prefix("namespace") and node[node.safeLen - 1].kind == nkStmtList:
      state.statement_namespace(node)
      return
  case node.kind
